#include "hkdf_sha256_test.h"

void start() {

    for (size_t i = 0; i < 3; i += 1) {
        u8 out[32];
        hkdf_extract_sha256(out, tests[i].extract_key_in, tests[i].salt);
        assert(arrays_equal(out, tests[i].key_in.items, 32));

        u8 key_out[82];
        hkdf_expand_sha256(key_out, tests[i].key_out_length, tests[i].key_in, tests[i].info);
        assert(arrays_equal(key_out, tests[i].key_out, tests[i].key_out_length));
    }
...
test tests[3] = {
    {
        ...
        (u8[]) {
            0x3c, 0xb2, 0x5f, 0x25, 0xfa, 0xac, 0xd5, 
    ...
    },
    {
        ...
        (u8[]) {
            0x8d, 0xa4, 0xe7, 0x75, 0xa5, 0x63, 0xc1, 
            0x8f, 0x71, 0x5f, 0x80, 0x2a, 0x06, 0x3c, 
            0x5a, 0x31, 0xb8, 0xa1, 0x1f, 0x5c, 0x5e, 
            0xe1, 0x87, 0x9e, 0xc3, 0x45, 0x4e, 0x5f, 
            0x3c, 0x73, 0x8d, 0x2d, 0x9d, 0x20, 0x13, 
            0x95, 0xfa, 0xa4, 0xb6, 0x1a, 0x96, 0xc8
        },
        42
    }
};